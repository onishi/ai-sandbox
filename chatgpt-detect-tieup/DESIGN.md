# chatgpt-detect-tieup 設計書

## 概要

曲名とアーティスト名を入力として、OpenAI API (ChatGPT) を使用してタイアップ情報（アニメ主題歌、ドラマ主題歌、CM曲など）および作詞者・作曲者情報を取得するスクリプト。
...
# 基本的な使い方（デフォルト: gpt-4o-mini）
npx ts-node src/index.ts "紅蓮華" "LiSA"

# モデルを指定 (4o または 4o-mini)
...
複数の曲を複数のモデルで実行し、結果を比較するベンチマークスクリプト。
...
各曲を 2パターン（gpt-4o-mini, gpt-4o）で実行：

```
model	song	artist	lyricist	composer	tieups	elapsed_ms	error
gpt-4o-mini	紅蓮華	LiSA	LiSA	草野華余子	アニメOP: 鬼滅の刃 (2019)	1234
gpt-4o	紅蓮華	LiSA	LiSA	草野華余子	アニメOP: 鬼滅の刃 (2019)	2456
```
...
| カラム | 説明 |
|--------|------|
| model | 使用モデル (gpt-4o-mini / gpt-4o) |
| song | 曲名 |
| artist | アーティスト名 |
| lyricist | 作詞者 |
| composer | 作曲者 |
...
- OpenAI API の回答は必ずしも正確ではない可能性がある（LLM の特性上）
- API キーは環境変数で管理し、コードにハードコードしない
- ベンチマーク実行時は API 呼び出し回数に注意（曲数 × 2回）
- API呼び出し間隔は 500ms
