...
const MODEL_VERSIONS: Array<"4o" | "4o-mini"> = ["4o-mini", "4o"];
...
async function runBenchmark(
  ...
  for (const { song, artist } of songs) {
    for (const modelVersion of MODEL_VERSIONS) {
      const options: Options = { model: modelVersion };

      console.error(
        `実行中: ${song} / ${artist} (model: ${modelVersion})`
      );
      ...
      try {
        ...
        results.push({
          song,
          artist,
          model: modelVersion === "4o" ? "gpt-4o" : "gpt-4o-mini",
          lyricist: "",
          composer: "",
          tieups: "",
          elapsedMs: 0,
